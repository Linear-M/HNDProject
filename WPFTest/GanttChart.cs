...
namespace GanttChart
{
    public class GanttChart : UserControl
    {
        ...
        private Font rowTextFont = new Font(UI.fontCollection.Families[0], 14, FontStyle.Regular);
        private Font dateTextFont = new Font(UI.fontCollection.Families[0], 8, FontStyle.Regular);
        private Font timeTextFont = new Font(UI.fontCollection.Families[0], 12, FontStyle.Regular);
            ...
            foreach (ChartBarDate bar in bars)
            {
                ...
                if (bar.StartValue != null & endValue != null)
                {
                    ...
                    if ((index >= scrollPos & index < barsViewable + scrollPos) | ignoreScrollAndMousePosition == true)
                    {
                        ...
                        //Date font, strings and sizes
                        Font dateFont = new Font(UI.fontCollection.Families[0], 12f);
                        string startString = bar.StartValue.Day + "/" + bar.StartValue.Month;
                        string endString = bar.EndValue.Day + "/" + bar.EndValue.Month;
                        SizeF endStringSize = grfx.MeasureString(endString, DateFont);
                        float scalingFactor = 1f;

                        grfx.DrawString(startString, dateFont, Brushes.White, obRect.X, obRect.Y);
                        ...
                        grfx.DrawString(endString, dateFont, Brushes.White, (((obRect.X + c) - (endStringSize.Width) * scalingFactor)), ((obRect.Y + d) - (endStringSize.Height) * scalingFactor));
        ...
        public void DrawNetVertical(Graphics grfx)
        {
            ...
            foreach (Header header in shownHeaderList)
            {
                ...
                grfx.DrawLine(Pens.Black, barStartLeft + (index * widthPerItem), headerLocationY, barStartLeft + (index * widthPerItem), lastLineStop);
                index += 1;
